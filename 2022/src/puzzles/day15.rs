...
struct Sensor {
    ...
    beacon_x: i64,
    beacon_y: i64,
    possible_locations: Vec<(i64, i64)>,
}
...
fn solve_b_for(input: &str, range: i64) -> i64 {
    ...
    for line in input.lines() {
        ...
        for i in 0..possible_distance {
            possible_locations.push((sensor_x + possible_distance - i, sensor_y + i));
            possible_locations.push((sensor_x - i, sensor_y + possible_distance - i));
            possible_locations.push((sensor_x - possible_distance + i, sensor_y - i));
            possible_locations.push((sensor_x + i, sensor_y - possible_distance + i));
        }
    ...
    for sensor in &sensors {
        for (x, y) in &sensor.possible_locations {
